...
@onready var road_layer = $"../../RoadsLayer"
...
var entrances_data = preload("res://client/scripts/map/entrances.gd").new()
...
func handle_manual_entrance_check() -> void:
	if Input.is_action_just_pressed("ui_accept"):
		check_for_entrance(road_layer.local_to_map(position))

func handle_mouse_click(mouse_position: Vector2) -> void:
	var local_position = road_layer.to_local(mouse_position)
	var clicked_cell = road_layer.local_to_map(local_position)
	var current_cell = road_layer.local_to_map(position)
...
func populate_astar() -> void:
	var used_cells = road_layer.get_used_cells()
...
func is_valid_move(cell: Vector2i) -> bool:
	return road_layer.get_cell_source_id(cell) != -1

func cell_to_id(cell: Vector2i) -> int:
	return cell.x + cell.y * road_layer.get_used_rect().size.x
...
func process_path_movement(delta: float) -> void:
	if current_path_index < path.size():
		target_position = road_layer.map_to_local(path[current_path_index])
		...
		if position.distance_to(target_position) < 1:
			...
			if current_path_index < path.size():
				var next_position = road_layer.map_to_local(path[current_path_index])
		...
		if is_local_player:
			player_manager.notify_of_position(player_id, position)
	else:
		path.clear()
		is_moving = false
		animated_sprite.play("idle")
...
func start_keyboard_movement(target_cell: Vector2i) -> void:
	target_position = road_layer.map_to_local(target_cell)
	is_moving = true
	can_move = false
	move_timer.start()
	animated_sprite.play("walk")
